...
// CREATE ENCODING
$encoding = new Encoding('PlayReady DRM Encoding Example');
...
$cencDrm1080p = createCencDrm($cencDrmKey, $cencDrmKid, array($cencDrmEncodingOutput1080p), null, $playreadyLaUrl);
$cencDrm720p = createCencDrm($cencDrmKey, $cencDrmKid, array($cencDrmEncodingOutput720p), null, $playreadyLaUrl);
$cencDrm480p = createCencDrm($cencDrmKey, $cencDrmKid, array($cencDrmEncodingOutput480p), null, $playreadyLaUrl);
$cencDrm360p = createCencDrm($cencDrmKey, $cencDrmKid, array($cencDrmEncodingOutput360p), null, $playreadyLaUrl);
$cencDrm240p = createCencDrm($cencDrmKey, $cencDrmKid, array($cencDrmEncodingOutput240p), null, $playreadyLaUrl);
 ...
 */
function createFmp4Muxing($apiClient, $encoding, $stream, $s3Output, $outputPath, $outputAcl = AclPermission::ACL_PUBLIC_READ, $initSegmentName = 'init.mp4', $segmentDuration = 4, $segmentNaming = 'segment_%number%.m4s')
{
...
function createEncodingOutput(Output $s3Output, $outputPath, $acl = AclPermission::ACL_PUBLIC_READ)
{
    $encodingOutput = new EncodingOutput($s3Output);
    $encodingOutput->setOutputPath($outputPath);
